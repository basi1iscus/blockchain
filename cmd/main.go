...
func RunScriptFromCode(c *gin.Context) {
	script := Script{}
	if err := c.ShouldBind(&script); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
	...
	fullScript := script.ScriptSig + "\n" + script.ScriptPubKey
	signer := sign_ed25519.Ed25519Signer{}
	vm := script_vm.New(&signer)
	byteCode, err := vm.ParseString(fullScript)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
	...
	scriptCode, err := runScript(vm, byteCode, signedData)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"message": fmt.Sprintf("%v", err),
...
func main() {
	...
	api.POST("/run_sript/binary", RunScript)
	api.POST("/run_sript/code", RunScriptFromCode)
	router.Run()
}
